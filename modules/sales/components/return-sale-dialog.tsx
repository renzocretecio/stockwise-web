...
import { useCreateSaleReturn, useSale } from "@/modules/sales/services/sales";
    ...
    <Dialog open={open} onOpenChange={handleOpenChange}>
      <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-3xl">
        <DialogHeader>
          ...
          <DialogDescription>
            Select quantities from {sale?.reference_number || "this sale"}.
            Returned stock is restored immediately.
          </DialogDescription>
        </DialogHeader>
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div className="space-y-2">
              {items.map((item) => {
                ...
                return (
                  <div
                    key={item.id}
                    className="grid gap-3 rounded-xl border p-3 sm:grid-cols-[1fr_130px_130px] sm:items-center"
                  >
                    <div>
                      <p className="font-medium">{item.product_name}</p>
                      <p className="text-xs text-muted-foreground">
                        {item.sku || "No SKU"} · Sold {item.quantity} · Already
          ...
          <div className="flex items-center justify-between rounded-xl bg-muted/50 px-4 py-3">
            ...
            <span className="text-lg font-semibold">
              {currency.format(refundEstimate)}
            </span>
          </div>
