...
export function Header() {
  ...
  const userRole =
    activeBusiness?.role || authData?.role || user?.role || "member";
  ...
  const handleBusinessChange = async (businessId: string) => {
    ...
    try {
      ...
      if (!response.ok || !result.active_business) {
        throw new Error("Unable to switch business");
      }
      const nextBusiness = result.active_business;
      await resetSessionCache(queryClient);
      queryClient.setQueryData(authKeys.me, {
        ...authData,
        active_business: nextBusiness,
        business_id: nextBusiness.id,
        business_name: nextBusiness.name,
        permissions: nextBusiness.permissions ?? [],
      });
  ...
  const isItemActive = (item: MenuItem): boolean => {
    ...
    if (item.children) {
      return item.children.some(
        (child) => child.href && isLinkActive(child.href, item.children),
      );
    }
    return false;
  };
              ...
              if (hasChildren) {
                return (
                  <div key={item.label} className="relative">
                    ...
                    >
                      {active && (
                        <span className="size-1.5 rounded-2xl bg-primary" />
                      )}
            ...
            <div ref={profileRef} className="relative">
              ...
              >
                ...
                <div className="hidden min-w-0 max-w-36 text-left xl:block">
                  ...
                  >
                    {activeBusiness?.name
                      ? `${activeBusiness.name} · ${subscriptionLabel(
                          activeBusiness.plan,
                ...
                >
                    ...
                    {(authData?.businesses?.length ?? 0) > 1 && (
                      <label className="mt-3 block text-xs font-medium">
                        ...
                        <select
                          ...
                          className="h-8 w-full rounded-2xl border bg-background px-2"
                        >
                  ...
                  <div className="space-y-0.5">
                    ...
                    <button
                      ...
                      className="flex w-full cursor-pointer items-center gap-2.5 rounded-lg px-3 py-2 text-left text-sm text-muted-foreground transition-colors hover:bg-muted hover:text-foreground"
                    >
            ...
            >
              ...
              <DrawerContent className="m-0! w-screen! max-w-none rounded-none">
                <DrawerHeader className="relative border-b px-5 pb-4 text-left">
                  ...
                  <DrawerDescription>{userRole} workspace</DrawerDescription>
                  <DrawerClose
                    render={
                      <button
