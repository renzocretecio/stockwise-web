    ...
    return (
        <div>
            ...
            >
                ...
                >
                    ...
                    {data.status === "trialing"
                        ? "Free for 14 days"
                        : formatPlanPrice(data.monthly_price_php)}
                </span>
            </header>
            ...
            <div className="border-t p-5">
                ...
                <div
                    className={
                        "mt-3 grid gap-x-6 gap-y-2 text-sm " + "sm:grid-cols-2"
                    }
                >
                    <Feature
                    ...
                    />
                    <Feature
                        included={data.features.offline_sync}
                        label="Offline mode"
                    />
                    <Feature
...
function TrialNotice({ subscription }: { subscription: SubscriptionSummary }) {
    ...
    if (subscription.status === "trialing") {
        const affectedMembers = Math.max(
            subscription.usage.active_members - 1,
            0,
        );
        ...
        return (
            <div className="border-b bg-primary/5 p-5 text-sm">
                ...
                <p className="mt-1 text-muted-foreground">
                    It ends {formatDate(subscription.trial_ends_at)}. After
                    that, this business returns to Free unless Pro is activated.
                </p>
                {affectedMembers > 0 ? (
                    <p
                        className={
                    ...
                    >
                        ...
                        {affectedMembers === 1 ? "member" : "members"} will
                        lose access when the trial ends.
                    </p>
                ) : null}
            </div>
        );
    ...
    if (subscription.status === "expired") {
        return (
            <div className="border-b bg-amber-500/10 p-5 text-sm">
                <p className="font-medium">Your Pro trial has ended.</p>
                <p className="mt-1 text-muted-foreground">
        ...
        <div className="border-t p-5">
            <div className="flex flex-wrap justify-between gap-3">
                <div>
                    ...
                    <p className="mt-1 text-xs text-muted-foreground">
                        {included} included ·{" "}
                        {subscription.additional_member_seats} of {addOnLimit}
                        {" additional seats added"}
                    </p>
                </div>
                <p className="text-sm font-semibold">
                    ₱{subscription.additional_member_price_php ?? 79}
