...
export function DashboardAssemblyHero() {
  ...
  useEffect(() => {
    ...
    const render = () => {
      ...
      const sectionDistance = section.offsetHeight - window.innerHeight;
      const progress =
      ...
      if (floatingHeaderRef.current) {
        floatingHeaderRef.current.style.opacity = String(floatingEntry);
      ...
      if (stageRef.current) {
        ...
        stageRef.current.style.transform =
          `translate3d(-50%, calc(-50% + ${rise}px), 0) ` + `scale(${scale})`;
      }

      if (templateRef.current) {
        templateRef.current.style.opacity = String(templateReveal);
      }
      ...
      pieceRefs.current.forEach((element, index) => {
        ...
        const scale = piece.scale + (1 - piece.scale) * assembly;
        ...
        element.style.filter =
          `drop-shadow(0 18px 28px ` + `rgb(23 33 38 / ${shadowOpacity}))`;
    ...
    <>
      ...
      <section
        className={
          ...
          "bg-background text-foreground xl:block " +
...
function MarketingHeader({
  ...
  return (
    ...
    >
      <BrandLink />
      <nav aria-label="Landing page sections" className="hidden md:block">
        <SectionLinks />
      </nav>
      <NavigationActions />
    </header>
    ...
    <ul className="flex items-center gap-0.5">
      {sectionLinks.map((link) => (
        <li key={link.href}>
          <Link
            className={cn(
              "block rounded-2xl font-medium text-muted-foreground",
...
function NavigationActions({ compact = false }: { compact?: boolean }) {
  return (
    <div className="flex items-center gap-2">
      <Link
        className={cn(
          "rounded-2xl font-medium text-accent-foreground",
          "hover:bg-muted",
      ...
      <Link
        className={cn(
          "rounded-2xl bg-primary font-medium text-primary-foreground",
          "hover:bg-primary/85",
          compact ? "px-4 py-2 text-sm" : "px-4 py-2 text-sm",
        )}
        href="/signup"
...
function HeroCopy({
  ...
  return (
    ...
    >
      <div
        className={"w-full max-w-5xl will-change-[opacity,transform]"}
        ref={introRef}
      >
        <p
          className={
        ...
        >
          KitaStock brings sales, inventory, and practical AI guidance into one
          workspace built for everyday business decisions.
        </p>
        <HeroActions />
      </div>
    </div>
  ...
  return (
    <div className="mt-8 flex flex-col items-center gap-4">
      <div className="flex flex-wrap justify-center gap-3">
        ...
        <Link
          className={
            "rounded-2xl border border-border bg-secondary/80 " +
...
function DashboardCanvas({
  ...
  pieceRefs?: React.MutableRefObject<Array<HTMLDivElement | null>>;
    ...
    <figure>
      ...
      >
        ...
        {pieces.map((piece, index) => (
          <div
            className={"absolute overflow-hidden rounded-2xl bg-[#ffffff]"}
            key={piece.src}
            ref={(element) => {
              if (pieceRefs) {
          ...
          >
            ...
            >
              ...
              {piece.contentHeight ? (
                <span
                  aria-hidden="true"
                  className={"absolute inset-x-0 bottom-0 h-1 " + "bg-[#ffff]"}
                />
              ) : null}
            </div>
          </div>
      ...
      <figcaption
        className={
          "mt-4 text-center text-xs font-medium " + "text-muted-foreground"
        }
      >
    ...
    <div
      ...
      style={{
        background:
          ...
          "hsl(45 90% 52% / 0.2), transparent 72%)",
      }}
    />
  );
}
...
function StaticHero() {
  return (
    <section
      className={
        "relative overflow-hidden bg-background px-4 pb-16 " +
        "pt-24 text-foreground xl:hidden motion-reduce:block"
      }
    >
      <HeroBottomGradient />
      <MarketingHeader />
      <div className="relative mx-auto max-w-3xl text-center">
        ...
        <p
          className={
            "mx-auto mt-5 max-w-xl text-sm leading-6 " + "text-muted-foreground"
          }
        >
          Sales, inventory, and practical guidance in one clear workspace.
        </p>
        <HeroActions />
      ...
      >
        ...
        <div
          className={
            ...
            "border-white/20 bg-card shadow-2xl " +
            "shadow-foreground/25 sm:left-auto sm:right-0 " +
            "sm:w-[24%]"
          }
        >
          <Image
