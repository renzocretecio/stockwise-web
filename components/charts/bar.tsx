...
export interface BarProps {
  ...
  dataKey: string;
  /** Y-scale group id for vertical bars. Default: `"left"`. */
  yAxisId?: string | number;
  /** Fill color or a per-datum color resolver. */
  fill?: BarFill;
    ...
    <g className={`bar-series-${uniqueId}`}>
      {data.map((d, i) => {
        ...
        if (isHorizontal) {
          ...
          const baselineX = scale(0) ?? 0;
          originX = baselineX;
          x = Math.min(baselineX, valuePos);
          barW = Math.abs(valuePos - baselineX);
          barHeight = barWidth;

          if (stacked && stackOffsets) {
            const offset = stackOffsets.get(i)?.get(dataKey) ?? 0;
            const startX = scale(offset) ?? baselineX;
          ...
          originY = baselineY;
          y = Math.min(baselineY, valuePos);
          barHeight = Math.abs(baselineY - valuePos);
          barW = barWidth;

          if (stacked && stackOffsets) {
            const offset = stackOffsets.get(i)?.get(dataKey) ?? 0;
            const startY = scale(offset) ?? baselineY;
            const endY = scale(offset + value) ?? startY;
            originY = startY;
            y = Math.min(startY, endY);
            barHeight = Math.abs(endY - startY);
            // Apply stack gap: shift up and reduce height
            const gapOffset = seriesIndex * stackGap;
            y -= value >= 0 ? gapOffset : -gapOffset;
        ...
        const resolvedFill =
          typeof fill === "function" ? fill(d, i) : fill;
