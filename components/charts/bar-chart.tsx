...
const ChartCore = memo(function ChartCore({
  ...
  // Include negative values so report charts can show outflows and losses.
  const valueExtent = useMemo(() => {
    ...
    return max === 0 && min === 0 ? { min: 0, max: 100 } : { min, max };
  }, [data, lines, stacked]);

  // Value scale (linear) - for the value axis
  const valueScale = useMemo(() => {
    const range = isHorizontal ? [0, innerWidth] : [innerHeight, 0];
    const min = valueExtent.min < 0 ? valueExtent.min * 1.1 : 0;
    const max = valueExtent.max > 0 ? valueExtent.max * 1.1 : 0;
    return scaleLinear({
      range,
      domain: [min, max],
      nice: true,
  ...
  const stackOffsets = useMemo(() => {
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      for (const line of lines) {
        const value = d[line.dataKey];
        if (typeof value === "number") {
          const offset = value >= 0 ? positive : negative;
          pointOffsets.set(line.dataKey, offset);
          if (value >= 0) {
            positive += value;
          } else {
            negative += value;
