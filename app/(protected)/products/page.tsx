...
export default function ProductsPage() {
    ...
    const activeSkuLimit = subscription.data?.limits.active_sku_limit;
    const activeSkuCount = subscription.data?.usage.active_skus ?? 0;
    const activeSkuLimitReached = Boolean(
        activeSkuLimit !== null &&
            activeSkuLimit !== undefined &&
            activeSkuCount >= activeSkuLimit,
    );
    ...
    const requestProductCreation = (action: "create" | "import") => {
        if (activeSkuLimitReached) {
            requestPlanUpgrade({
                code: "active_sku_limit_reached",
                limit: activeSkuLimit ?? undefined,
                message:
                    `Your current plan allows ${activeSkuLimit} active SKUs. ` +
                    "Upgrade to add more products.",
                title: "Active SKU limit reached",
            });
            return;
        }
        ...
        <div className="pb-12">
            ...
            >
                ...
                <div className="flex flex-wrap items-center gap-2">
                    ...
                    {canCreate ? (
                        <Button
                            onClick={() => requestProductCreation("import")}
                            size="sm"
                            type="button"
                            variant="outline"
                        >
                    ...
                    <Button
                        onClick={() => requestProductCreation("create")}
                        size="sm"
                        type="button"
                    >
